...
public class QueryUtils {
    ...
    private static final String LOG_TAG = "HARSH";
    ...
    private static List<News> extractFeatureFromJson(String jsonResponse){
        ...
        String title,content,date,author,sectionName,url;
        ...
        try{
            ...
            for(int i=0; i< results.length(); i++){
                ...
                content = fields.getString("bodyText");
                StringBuilder auth = new StringBuilder();

                JSONArray tags = object.getJSONArray("tags");
                for(int j=0; j<tags.length();j++){
                    JSONObject tagsObj = tags.getJSONObject(j);
                    if(tagsObj.getString("type").equals("contributor")){
                        auth.append(tagsObj.getString("webTitle"));
                    }
                }

                author = auth.toString();

                news.add(new News(title,content,date,author,sectionName,url));
            }

        }catch(JSONException e){
            Log.e(LOG_TAG,"Error while parsing the json result",e);
        }
